...
- see if we already have sources in `./docs` (project relative)
  - IF NOT use `mcp__plugin_kodegen_kodegen__git_clone` to clone them into `./tmp/` (project relative) and explore the docs, examples and sources with sequential thinking.
...
- use `mcp__plugin_kodegen_kodegen__git_clone` to clone repositories into `./tmp` for research
