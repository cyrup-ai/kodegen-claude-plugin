...
- use `mcp__plugin_kodegen_kodegen__sequential_thinking` and ULTRATHINK to think step by step about the code review
- use `mcp__plugin_kodegen_kodegen__browser_web_search` if research on the web is needed for the code review
- use `mcp__plugin_kodegen_kodegen__scrape_url` if you find websites that are key to understanding the code to scrape the full website
...
- use `mcp__plugin_kodegen_kodegen__fs_write_file` to write tasks for any issues discovered during review
  - if you need to make additional edits to the code review tasks periodically, use `mcp__plugin_kodegen_kodegen__fs_edit_block` to make those edits
