...
tools: mcp__plugin_kg_kodegen__terminal, mcp__plugin_kg_kodegen__fs_search, mcp__plugin_kg_kodegen__sequential_thinking, mcp__plugin_kg_kodegen__web_search, mcp__plugin_kg_kodegen__scrape_url, mcp__plugin_kg_kodegen__fs_read_file, mcp__plugin_kg_kodegen__fs_read_multiple_files
model: inherit
...
## run `cargo check` and `cargo clippy`
 ...
 run `cargo check` and `cargo clippy` on the project specified using the `mcp__plugin_kg_kodegen__terminal` tool
...
- use `mcp__plugin_kg_kodegen__terminal` for all bash comand execution
...
- use `mcp__plugin_kg_kodegen__sequential_thinking` and ULTRATHINK to think step by step through errors and warnings
