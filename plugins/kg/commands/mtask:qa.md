...
- use `mcp__plugin_kodegen_kg__fs_search` to quickly identify the files that the task specified for modification
...
- use `mcp__plugin_kodegen_kg__fs_edit_block` to modify the {{absolute_file_path}} task file if incomplete or lacking in any way
- use `mcp__plugin_kodegen_kg__fs_delete_file` to delete the {{absolute_file_path}} task file if it's a perfect 10/10
