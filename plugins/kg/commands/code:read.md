...
- see if we already have sources in `./docs` (project relative)
  - IF NOT use `mcp__plugin_kodegen_kg__git_clone` to clone them into `./tmp/` (project relative) and explore the docs, examples and sources with sequential thinking.

## TOOLS 

- use `mcp__plugin_kodegen_kg__sequential_thinking` and ULTRATHINK to think step by step about the task
- use `mcp__plugin_kodegen_kg__browser_web_search` if research on the web is needed for the task scope
- use `mcp__plugin_kodegen_kg__scrape_url` if you find websites that are key to understanding the task to scrape the full website
  - once completed use `mcp__plugin_kodegen_kg__scrape_search_results` to quickly find the most relevant information from the crawl results
...
- use `mcp__plugin_kodegen_kg__git_clone` to clone repositories into `./tmp` for research
- use these tools to explore github directly:
  - `mcp__plugin_kodegen_kg__github_get_file_contents`
  - `mcp__plugin_kodegen_kg__github_search_code`
  - `mcp__plugin_kodegen_kg__github_search_repositories`
  - `mcp__plugin_kodegen_kg__github_search_issues`
